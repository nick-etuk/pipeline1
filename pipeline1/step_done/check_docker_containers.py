...
def check_docker(object_type: str, step: dict[str, Any], calling_function: str) -> bool:
    ...
    if object_type == 'containers':
        ...
        log.warn(f"Unknown docker check type: {object_type}")
        return False
    ...
    if len(missing_items) > 0:
        ...
        if not calling_function == 'wait_for_parallel':
            item_list = "\n".join(missing_items)
            log.warn(f"Missing Docker {object_type} for {step['stepId']}:")
            log.warn(f"{item_list}")
        return False
    ...
    if len(unexpected_items) > 0:
        ...
        if not calling_function == 'wait_for_parallel':
            ...
            # warn(f"Unexpected Docker {object_type} for {step['stepId']}:")
            # warn(f"{item_list}")
